...
ax.add_patch(descartes.PolygonPatch(small, fc='r', ec='k',
                                    label=r'Low Ambiguity'))
ax.add_patch(descartes.PolygonPatch(middle, fc='orange', ec='k',
                                    label=r'High Ambiguity'))

# Initialize and format plot.
plt.ylim((-0.10, 0.10)); plt.xlim((-0.10, 0.10))
...
plt.axvline(x=0.00, ymin=0.0, ymax=0.50, linewidth=1, color='grey',
            linestyle='--')
...
# Save.
plt.savefig('../exploring_entropy.png')
